...
{
    double just_f = G * b.mass * this->mass / (distance(b) * distance(b));
    this->force.x += just_f * (std::abs(this->coords.x - b.coords.x)) / distance(b);
    this->force.y += just_f * (std::abs(this->coords.y - b.coords.y)) / distance(b);
}
